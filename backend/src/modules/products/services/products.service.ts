...
export class ProductService {
  ...
  async findByMainCategory(
    ...
    const data = await this.productModel
      .find(query)
      .select('name slug basePrice thumbnail publishedAt createdAt') // Chỉ lấy các trường cần thiết
  ...
  async findBySubCategory(
    ...
    const [total, data] = await Promise.all([
      ...
      this.productModel
        .find(
          query,
          'name slug basePrice thumbnail publishedAt createdAt',
        )
  ...
  async findAllBasicInfo(
    ...
    const data = await this.productModel
      .find()
      .select('name slug basePrice thumbnail category publishedAt createdAt')
